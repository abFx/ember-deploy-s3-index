...
module.exports = CoreObject.extend({
  ...
  */
  _getWebsiteParams: function(revision){
   ...
   /**
  * Saves local bucket index file in db
  * @param {string} client_id - client web app ID
  * @param {string} client_secret - client web app ID
  * @param {string} version_code - fingerprint code
  * @param {string} app_code - 1,2,3,4 depending on type of store
  * @returns NOTHING
  */
  _updateIndex: function(client_id, client_secret, version_code, app_code){
    var queryData = {
      'client_id'     : client_id,
    ...
    var options = {
      host: 'http://api.development.naranyamarket.com',
      path: '/store_config_front_end_config_update',
      port: 3000,
      method: 'PUT',
      //headers: headers
    };

    var saveIndex = http.request(options, function(res) {
